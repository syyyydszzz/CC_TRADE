...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--buildforce-root")
    parser.add_argument("--artifact-path", required=True)
    parser.add_argument("--json", action="store_true", dest="json_mode")
    return parser.parse_args()


def resolve_artifact(buildforce_root: Path, raw_path: str) -> Path:
    path_obj = Path(raw_path)
    candidates = [path_obj]
    if not path_obj.is_absolute():
        candidates.insert(0, buildforce_root / path_obj)
...
def repo_relative_or_absolute(buildforce_root: Path, path: Path) -> str:
    try:
        return path.relative_to(buildforce_root).as_posix()
    except ValueError:
        return str(path)


def extract_summary(payload: Dict[str, Any], artifact_path: Path, buildforce_root: Path) -> Dict[str, Any]:
    summary = extract_metrics_summary(payload)
    summary["artifact_path"] = repo_relative_or_absolute(buildforce_root, artifact_path)
    return summary
...
def main() -> int:
    args = parse_args()
    raw_root = args.repo_root or args.buildforce_root
    if raw_root is None:
        print("ERROR: --repo-root is required.", file=sys.stderr)
        return 2

    buildforce_root = Path(raw_root).expanduser().resolve()

    try:
        artifact_path = resolve_artifact(buildforce_root, args.artifact_path)
    ...
    summary = extract_summary(payload, artifact_path, buildforce_root)
