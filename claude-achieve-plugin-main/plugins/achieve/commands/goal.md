...
If the goal touches a strategy under `algorithms/`, also read:
- `.buildforce/context/result-workspace-contract.yaml`
- `.claude/skills/strategy-writer/SKILL.md` when it exists
- The local strategy `spec.md` and `results/` workspace before making result-layer decisions
- Initialize missing result workspaces with `bash .buildforce/scripts/bash/init-result-workspace.sh --strategy-path algorithms/{strategy}`
- After result updates, run `bash .buildforce/scripts/bash/check-result-workspace.sh --strategy-path algorithms/{strategy}` as a self-check
...
5. **Keep result authority separate**: For strategy work, store strategy results in `algorithms/{strategy}/results/`; do not treat Achieve session files as the strategy result record
