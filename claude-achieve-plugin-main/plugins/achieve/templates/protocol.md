...
Follow repository-level contracts when they exist.

## Repository-Aware Rules

### If the goal touches a strategy under `algorithms/`

- Read `.buildforce/context/result-workspace-contract.yaml` before planning result persistence.
- Read the local strategy `spec.md`, then `results/state.yaml`, then `results/report.md` when those files exist.
- If `results/` is missing, initialize it with `bash .buildforce/scripts/bash/init-result-workspace.sh --strategy-path algorithms/{strategy}` before treating the strategy workspace as ready.
- Use this result read order: `spec.md` -> `results/state.yaml` -> `results/report.md` -> latest `run.yaml` -> raw `*.json` only when needed.
- Use this result write order after execution:
  1. Write raw artifacts under `results/artifacts/runs/{run_id}/`
  2. Update `results/state.yaml`
  3. Refresh `results/report.md`
- After updating the result workspace, run `bash .buildforce/scripts/bash/check-result-workspace.sh --strategy-path algorithms/{strategy}` as a self-check and report any contract violations.
- Keep Achieve runtime authority in `.claude/achieve-sessions/**`.
- Do not copy Achieve `current_phase`, `iteration`, or `attempts` into the strategy result layer as authoritative fields.
- Keep BuildForce workflow artifacts under `.buildforce/sessions/**`. Sync only important conclusions there when useful.
...
5. If the goal touches a strategy, inspect the existing `spec.md`, `results/` workspace, and `.buildforce/context/result-workspace-contract.yaml` before planning changes
...
**If the goal touches a strategy under `algorithms/`:**
- Include `spec.md`, `results/state.yaml`, `results/report.md`, and expected run artifact paths in `files_to_modify` or `files_to_create` when applicable.
- Keep Achieve session files and strategy result files as separate layers with separate authority.
...
6. If the goal touches a strategy and you run compile, backtest, optimization, or validation:
   - Initialize `results/` first if it does not exist
   - Persist raw outputs under `results/artifacts/runs/{run_id}/`
   - Update `results/state.yaml` with the latest summary, decision, next actions, and any session refs
   - Refresh `results/report.md` with the concise current verdict
   - Run the result workspace self-check and surface any contract violations before moving on
...
2. If the goal touches a strategy, use `results/state.yaml`, `results/report.md`, and the latest `run.yaml` as the primary validation summary before drilling into raw JSON
3. Check each criterion explicitly
4. Document results
