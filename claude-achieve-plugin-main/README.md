...
This repository vendors the Achieve marketplace locally so the command and protocol files can be edited in place with the project.
...
```
/plugin marketplace add ./claude-achieve-plugin-main
```
...
When a goal touches a strategy under `algorithms/`, Achieve should treat the repository result workspace contract as authoritative for result persistence:

- Read `.buildforce/context/result-workspace-contract.yaml`
- Initialize missing workspaces with `bash .buildforce/scripts/bash/init-result-workspace.sh --strategy-path algorithms/{strategy}`
- Persist raw execution outputs under `results/artifacts/runs/{run_id}/`
- Update `results/state.yaml`
- Refresh `results/report.md`
- Run `bash .buildforce/scripts/bash/check-result-workspace.sh --strategy-path algorithms/{strategy}` as a self-check

Achieve runtime state still belongs under `.claude/achieve-sessions/`. BuildForce workflow artifacts still belong under `.buildforce/sessions/`.
