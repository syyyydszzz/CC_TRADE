...
3. **Follow the Plan**: Execute implementation steps sequentially as specified in the plan. Parse $ARGUMENTS for iteration-specific instructions (e.g., "change library X to Y", "fix edge case Z"). Reference specific file paths when creating or modifying code. Keep progress updates concise but informative.
   ...
   **Strategy result workspace rule**:

   - If the implementation touches a strategy under `algorithms/{strategy}/`, follow the project result contract in `.buildforce/context/result-workspace-contract.yaml` when it exists
   - Keep result persistence in the local `results/` workspace, not in BuildForce session files
   - If the strategy workspace does not have `results/` yet, initialize it with `bash .buildforce/scripts/bash/init-result-workspace.sh --strategy-path algorithms/{strategy}`
   - Write in this order:
     1. raw artifacts under `results/artifacts/runs/{run_id}/`
     2. `results/state.yaml`
     3. `results/report.md`
   - Do not copy Achieve runtime authority or BuildForce artifact authority into the result state; store only references when needed
   - After updating result files, run `bash .buildforce/scripts/bash/check-result-workspace.sh --strategy-path algorithms/{strategy}` and report any contract violations as result-layer issues
