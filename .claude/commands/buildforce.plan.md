...
2. **For CREATE mode (new spec)**:
   ...
   **Step 2d.1: Load project result contract context** (if available):

   - Check if `.buildforce/context/result-workspace-contract.yaml` exists
   - If it exists and the plan affects `algorithms/{strategy}/`, treat it as the canonical result-layer contract
   - Ensure the plan accounts for:
     - `results/state.yaml`
     - `results/report.md`
     - `results/artifacts/runs/{run_id}/`
   - Ensure any strategy workflow plan follows the write order:
     - raw artifacts
     - `results/state.yaml`
     - `results/report.md`
   ...
   **For plan.yaml (HOW to build)**:
   ...
   - If the work touches strategy directories under `algorithms/`, include any required result workspace files in `files_to_create` or `files_to_modify`
