...
class TestInferEntryFamily:
    ...
    def test_breakout_keywords_match(self, base_hypothesis_card: dict) -> None:
        card = _hypothesis(
            ...
            proposed_rule_changes=[
                {"component": "entry_logic", "change": "Require breakout above pivot after VCP pattern."}
            ],
        )
...
class TestCanExport:
    ...
    def test_pursue_no_family_returns_false(self, base_hypothesis_card: dict) -> None:
        card = _hypothesis(
            ...
            proposed_rule_changes=[
                {"component": "entry_logic", "change": "Fade extreme dislocations after opening spike."}
            ],
        )
        assert not can_export(card)
...
class TestValidateStrategyYaml:
    ...
    def test_candidate_contract_import_failure(self, base_hypothesis_card: dict, monkeypatch) -> None:
