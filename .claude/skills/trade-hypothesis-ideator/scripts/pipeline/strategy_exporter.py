...
CANDIDATE_CONTRACT_PATH = (
    Path(__file__).resolve().parents[3] / "edge-candidate-agent" / "scripts" / "candidate_contract.py"
)
...
def can_export(hypothesis: dict) -> bool:
    ...
    if not isinstance(hypothesis, dict):
        return False
    return hypothesis.get("recommendation") == "pursue" and infer_entry_family(hypothesis) is not None
...
def _load_candidate_contract_validator():
    if not CANDIDATE_CONTRACT_PATH.exists():
        raise RuntimeError(
            "candidate_contract.py not found at expected path: "
