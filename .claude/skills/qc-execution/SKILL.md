...
6. Run optimization or validation tools only when the user or the plan explicitly requires them.
7. Write raw `compile.json`, `backtest.json`, `optimization.json`, or `validation.json` under `results/artifacts/runs/{run_id}/` for downstream persistence.
8. Hand off to `result-workspace` for machine-readable persistence and to analysis skills for interpretation.
