...
def resolve_project_dir(repo_root: Path, strategy_path: Optional[str]) -> Path:
    if strategy_path:
        raise ValueError("Single-strategy repo always uses the repo root. Do not pass a separate project path.")

    if (repo_root / "main.py").exists():
        return repo_root

    raise ValueError(
        "Repo root does not look like a single-strategy workspace."
    )
...
def resolve_repo_asset(repo_root: Path, relative_path: str, legacy_path: Optional[str] = None) -> Path:
    primary = repo_root / relative_path
    if primary.exists():
        return primary
    if legacy_path is not None:
        fallback = repo_root / legacy_path
        if fallback.exists():
            return fallback
    raise ValueError(f"Required repository asset does not exist: {relative_path}")


def resolve_existing_reference(repo_root: Path, strategy_dir: Path, raw_path: str) -> Optional[Path]:
...
def validate_latest_section(
    ...
    if artifact is not None:
        ...
        elif resolve_existing_reference(repo_root, strategy_dir, artifact) is None:
...
def validate_run_yaml(
    ...
    if not run_yaml_path.exists():
        errors.append(f"Missing latest run summary: {as_repo_relative(run_yaml_path, repo_root)}")
        return None

    try:
        run_data = load_yaml(run_yaml_path)
    except Exception as exc:  # noqa: BLE001
        errors.append(f"Failed to parse {as_repo_relative(run_yaml_path, repo_root)}: {exc}")
        return None
...
def init_workspace(repo_root: Path, strategy_dir: Path, json_mode: bool, force: bool) -> int:
    ...
    template_state = load_yaml(
        resolve_repo_asset(repo_root, "workflow/templates/result-state-template.yaml")
    )
    ...
    for directory in (results_root, artifacts_root, runs_root):
        if directory.exists():
            skipped.append(as_repo_relative(directory, repo_root))
        else:
            directory.mkdir(parents=True, exist_ok=True)
            created.append(as_repo_relative(directory, repo_root))

    if state_path.exists() and not force:
        skipped.append(as_repo_relative(state_path, repo_root))
    else:
        if state_path.exists():
            overwritten.append(as_repo_relative(state_path, repo_root))
        else:
            created.append(as_repo_relative(state_path, repo_root))
        dump_yaml(state_path, state_payload)

    if report_path.exists() and not force:
        skipped.append(as_repo_relative(report_path, repo_root))
    else:
        if report_path.exists():
            overwritten.append(as_repo_relative(report_path, repo_root))
        else:
            created.append(as_repo_relative(report_path, repo_root))
        report_path.write_text(report_payload, encoding="utf-8")

    payload = {
        "strategy_path": as_repo_relative(strategy_dir, repo_root),
        "strategy_id": strategy_id,
        "results_root": as_repo_relative(results_root, repo_root),
        "created": created,
        "skipped": skipped,
...
def record_run(
    ...
    template = load_yaml(
        resolve_repo_asset(repo_root, "workflow/templates/result-run-template.yaml")
    )
    ...
    template["local_strategy_path"] = as_repo_relative(strategy_dir, repo_root)
    ...
    payload = {
        "strategy_path": as_repo_relative(strategy_dir, repo_root),
        "run_id": run_id,
        "run_yaml": as_repo_relative(run_yaml_path, repo_root),
...
def record_state(
    ...
    payload = {
        "strategy_path": as_repo_relative(strategy_dir, repo_root),
        "state_yaml": as_repo_relative(state_path, repo_root),
        "run_id": run_id,
        "status": status,
...
def validate_workspace(repo_root: Path, strategy_dir: Path, json_mode: bool) -> int:
    errors: List[str] = []
    warnings: List[str] = []
    ...
    if state_path.exists():
        try:
            ...
            errors.append(f"Failed to parse {as_repo_relative(state_path, repo_root)}: {exc}")
    ...
    if state_data:
        ...
        if not isinstance(refs, dict):
            ...
            if spec_ref is not None:
                ...
                elif resolve_existing_reference(repo_root, strategy_dir, spec_ref) is None:
            ...
            elif resolve_existing_reference(repo_root, strategy_dir, report_ref) is None:
                warnings.append(f"results/state.yaml.refs.report does not exist: {report_ref}")

            for ref_key in ("change", "runtime_session"):
        ...
        if latest_run_id is None:
            ...
            run_data = validate_run_yaml(repo_root, strategy_dir, run_dir, errors)
...
def main() -> int:
    ...
    try:
        strategy_dir = resolve_project_dir(repo_root, None)
    ...
    try:
        if args.command == "init":
            return init_workspace(repo_root, strategy_dir, args.json, args.force)
        if args.command == "check":
            return validate_workspace(repo_root, strategy_dir, args.json)
