...
This skill is a **deep-review layer** that sits alongside `qc-backtest-analysis`, not a replacement for it.

- `qc-backtest-analysis` AUTO-INVOKEs first for quick metric interpretation from `results/report.md`.
- Use `backtest-expert` when deeper scrutiny is needed: parameter sensitivity, sample size adequacy, stress testing assumptions, or when `qc-backtest-analysis` returns `needs_iteration` and the cause is unclear.
- Extract metrics from `results/artifacts/runs/{run_id}/backtest.json` and pass them as CLI args to `evaluate_backtest.py`.
- Output JSON goes to `results/artifacts/runs/{run_id}/backtest_eval.json` to keep it alongside QC artifacts.
- If verdict is Refine after multiple iterations with no improvement, trigger `strategy-pivot-designer` with the accumulated eval history.
- Do NOT run compile or backtest directly — that is `qc-execution`'s responsibility.
